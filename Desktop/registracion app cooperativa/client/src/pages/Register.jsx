...
import { EcosystemAlert } from '../components/EcosystemBadge';
...
const Register = () => {
    ...
    // Ecosystem account detection
    const [ecosystemAccount, setEcosystemAccount] = useState(null);
    const [checkingPhone, setCheckingPhone] = useState(false);

    // Cooperative selection - fetched from API
        ...
        const fetchCooperativas = async () => {
            try {
                ...
                const mappedData = data.map(c => ({
                    ...
                    name: c.nombre,
                    city: c.ciudad,
                    status: c.estado === 'activa' ? 'active' : c.estado === 'proximamente' ? 'coming_soon' : 'inactive'
                }));
                setCooperativas(mappedData);
            } catch (error) {
    ...
    const confirmFront = async () => {
        ...
        // Create AbortController with timeout to prevent infinite waiting
        ...
        try {
            ...
            clearTimeout(timeoutId);
            console.warn("⚠️ AI Front timeout or error:", e.name === 'AbortError' ? 'Timeout' : e.message);
        }
            ...
            if (found) {
                ...
                try {
                    ...
                    if (result.success && result.data?.dataFound) {
                        ...
                        setScanStatus('Listo - Continúa manualmente');
                    }
                } catch (err) {
                    clearTimeout(timeoutId);
                    console.warn("⚠️ Backend timeout or error:", err.name === 'AbortError' ? 'Timeout' : err.message);
                    setScanStatus('Listo - Ingresa datos manualmente');
                }
    ...
    // Check if phone exists in ecosystem (RAITE ID)
    const checkExistingAccount = async (phone) => {
        if (phone.length !== 10) return;

        setCheckingPhone(true);
        try {
            const response = await fetch(`/api/ecosystem/check-phone/${phone}`);
            // Safe JSON parsing
            const text = await response.text();
            const data = text ? JSON.parse(text) : {};

            if (data.exists) {
                setEcosystemAccount(data);
                console.log('🔗 Existing ecosystem account found:', data);
            } else {
                setEcosystemAccount(null);
            }
        } catch (err) {
            console.error('Error checking ecosystem:', err);
            setEcosystemAccount(null);
        } finally {
    ...
    const submitRegistration = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                cooperativa_id: selectedCooperativa,
                cooperativa_name: cooperativas.find(c => c.id === selectedCooperativa)?.name || '',
                images,
            ...
            <header className={`bg-white dark:bg-slate-900 shadow-sm border-b dark:border-slate-800 sticky top-0 z-50 transition-all duration-500 ${isLoaded ? 'translate-y-0 opacity-100' : '-translate-y-4 opacity-0'}`}>
                <div className="p-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[8px] text-gray-400 ml-1">v3.3 (Visual)</span>
                    </div>
                    <div className="w-10" />
                </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="text-sm font-medium text-gray-600 dark:text-gray-400 flex items-center gap-2">
                                        ...
                                        Cooperativa a la que te unes
                                    </label>
                                    <select
                                        value={selectedCooperativa}
                                        onChange={(e) => setSelectedCooperativa(e.target.value)}
                                        className={`w-full mt-1 px-4 py-3 rounded-xl border ${!selectedCooperativa ? 'border-orange-400 ring-2 ring-orange-200' : 'border-gray-200 dark:border-slate-700'} bg-gray-50 dark:bg-slate-800 dark:text-white appearance-none cursor-pointer`}
                                    >
                                        <option value="">-- Selecciona tu cooperativa --</option>
                                        {cooperativas.filter(c => c.status === 'active').map(coop => (
                                            <option key={coop.id} value={coop.id}>
                                                {coop.name} ({coop.city})
                                            </option>
                                        ))}
                                        <optgroup label="Próximamente">
                                            {cooperativas.filter(c => c.status === 'coming_soon').map(coop => (
                                                <option key={coop.id} value={coop.id} disabled>
                                                    {coop.name} - Próximamente
                                                </option>
                                            ))}
                                        </optgroup>
                                    </select>
                                    {!selectedCooperativa && (
                                        <p className="text-orange-500 text-xs mt-1 flex items-center gap-1">
                                            <AlertCircle size={12} />
                                            Debes seleccionar una cooperativa
                                        </p>
                                    )}
                                </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border dark:border-slate-800 shadow-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    {checkingPhone && <p className="text-blue-500 text-xs mt-1 animate-pulse">Verificando en ecosistema...</p>}
                                </div>

                                {/* Ecosystem Account Alert */}
                                {ecosystemAccount && (
                                    <EcosystemAlert
                                        userName={ecosystemAccount.user?.name}
                                        apps={ecosystemAccount.apps}
                                        onContinue={() => {
                                            // Pre-fill data from ecosystem if available
                                            if (ecosystemAccount.user?.name && !scannedData?.fullName) {
                                                setScannedData(prev => ({ ...prev, fullName: ecosystemAccount.user.name }));
                                            }
                                        }}
                                    />
                                )}
