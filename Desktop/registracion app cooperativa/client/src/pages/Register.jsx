...
const Register = () => {
    ...
    const processImages = async (finalImages) => {
        ...
        try {
            // Initialize Tesseract worker with Spanish
            const worker = await createWorker('spa');
            console.log("✅ Worker Tesseract inicializado");

            // Process FRONT of INE
            console.log("📄 Procesando INE Frente...");
            const frontResult = await worker.recognize(finalImages.front);
            ...
            const backResult = await worker.recognize(finalImages.back);
            const backText = backResult.data.text;
            console.log("📝 Texto OCR (Reverso):", backText);
    ...
    const extractINEData = (frontText, backText) => {
        ...
        // ===== NOMBRE EXTRACTION =====
        // Pattern 1: "NOMBRE" followed by name
        let fullName = frontText.match(/NOMBRE\s*[:\s]*([A-ZÁÉÍÓÚÑ\s]+)/i)?.[1]?.trim();

        // Pattern 2: Look for typical name patterns (APELLIDO PATERNO, APELLIDO MATERNO, NOMBRE)
        if (!fullName) {
            const apellidoPaterno = frontText.match(/APELLIDO\s*PATERNO[:\s]*([A-ZÁÉÍÓÚÑ]+)/i)?.[1];
            const apellidoMaterno = frontText.match(/APELLIDO\s*MATERNO[:\s]*([A-ZÁÉÍÓÚÑ]+)/i)?.[1];
            const nombre = frontText.match(/NOMBRE\(?S?\)?[:\s]*([A-ZÁÉÍÓÚÑ\s]+)/i)?.[1];
            if (apellidoPaterno || nombre) {
                fullName = [apellidoPaterno, apellidoMaterno, nombre].filter(Boolean).join(" ");
            }
        }

        // Pattern 3: Any sequence of 2-4 capitalized words 
        if (!fullName) {
            const nameMatch = frontText.match(/([A-ZÁÉÍÓÚÑ]{2,}\s+[A-ZÁÉÍÓÚÑ]{2,}(?:\s+[A-ZÁÉÍÓÚÑ]{2,}){0,2})/);
            fullName = nameMatch?.[1]?.trim();
        }

        // ===== CURP EXTRACTION =====
        // Standard CURP format: 18 alphanumeric characters
        const curpPattern = /[A-Z]{4}\d{6}[HM][A-Z]{5}[A-Z0-9]\d/;
        let curp = allText.match(curpPattern)?.[0];

        // Alternative pattern (sometimes OCR misreads)
        if (!curp) {
        ...
        let address = backText.match(/DOMICILIO[:\s]*([^\n]+(?:\n[^\n]+)?)/i)?.[1]?.trim();
        if (!address) {
            address = backText.match(/CALLE[:\s]*([^\n]+)/i)?.[1]?.trim();
        }
        if (!address) {
            // Look for "C." or "AV." or "CALLE" patterns
            address = backText.match(/((?:C\.|AV\.|CALLE|BLVD\.?)[^\n]+)/i)?.[1]?.trim();
        }

        // ===== CLAVE DE ELECTOR =====
        const claveElector = allText.match(/CLAVE\s*DE\s*ELECTOR[:\s]*([A-Z0-9]{18})/i)?.[1];

        // ===== FECHA DE NACIMIENTO =====
        const fechaNac = frontText.match(/(\d{2}\/\d{2}\/\d{4})/)?.[1];

        // ===== SECCION =====
        const seccion = allText.match(/SECCI[OÓ]N[:\s]*(\d{4})/i)?.[1];
                ...
                {step > 0 && step < 5 && (
                    <div className="px-4 pb-4">
                            ...
                            {progressSteps.map((s, i) => (
                                <div key={s.id} className="flex items-center flex-1">
                                    ...
                                    {i < progressSteps.length - 1 && (
                                        <div className={`flex-1 h-1 mx-2 rounded transition-all duration-500 ${step > s.id ? 'bg-gradient-to-r from-orange-500 to-pink-500' : 'bg-gray-200 dark:bg-slate-800'}`} />
                                    )}
                                </div>
                            ))}
                        </div>
                        <div className="flex justify-between mt-2">
                            {progressSteps.map(s => (
                                <span key={s.id} className={`text-xs font-medium ${step >= s.id ? 'text-orange-500' : 'text-gray-400'}`}>
                                    {s.label}
                                </span>
                            ))}
                        </div>
